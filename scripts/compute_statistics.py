...
if __name__ == "__main__":
    ...
    for id in tqdm(ids, total=len(ids)):
        distance_map = np.load(f"../data/processed/{id}/distance_map_{args.jaw}.npy")
        distance_map_acc.append(distance_map)
    distance_map_acc = np.array(distance_map_acc)
    distance_dict = {}
    distance_map_mean = np.zeros((args.teeth, args.teeth, 3))
    distance_map_std = np.zeros((args.teeth, args.teeth, 3))
    for i in range(args.teeth):
        for j in range(args.teeth):
    ...
    np.save(f"../data/statistics/distance_map_mean_{args.jaw}.npy", distance_map_mean)
    np.save(f"../data/statistics/distance_map_std_{args.jaw}.npy", distance_map_std)
